...
const loadServer = async function(getServerFn, serverName) {
    ...
    const port = await promback(server.on.bind(server))("ready");
    ...
    await promback(socket.on.bind(socket))("connect");
...
const loadNewServers = async function() {
    ...
    await Promise.all([mainServer, backupServer]);
};
...
const initialiseConnectivity = async function(port = 8000) {
