...
function initalise(port, serverFunction, hostId, otherData) {
    ...
    io.on("connection", function(hostSocket) {
        ...
        serverIOEmitter.on("delete-user", (data, clientId) => {
            ...
            let socketClient = SocketClient.getClient(clientId);
            socketClient._sendEvent("disconnect", {});

            SocketClient.deleteClient(clientId);
        });

        serverIOEmitter.on("end-process", () => {
            logger.info("Killing server process");
            process.exit(7);
        });

        const eventListeners = {};
        serverIOEmitter.on("inner-event", data => {
            if (eventListeners[data.name]) {
                eventListeners[data.name].forEach(listener => listener(data.data));
            }
        });
...
process.on("message", message => {
    ...
    switch (cmd) {
        case "INFO":
            initalise(data.port, data.serverFunction, data.hostId, data.otherData);
            break;
        case "SET_NAME":
            setNameTo(data.name);
            break;
